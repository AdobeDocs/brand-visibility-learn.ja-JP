---
title: テストの概要
description: トークン、埋め込み、RAGなどのLLMの実際の仕組みと、AI 検索がSEOをGEOに移行している理由をご確認ください。 マーケターがAIから引用を入手するための実践的なガイド。
autotag-review: '2026-08-25T17:55:59.130Z'
TQID: 'https://experienceleague.adobe.com/j891oRQoiZfSO5whAmV25aYxEnDDjOD4JyWSoistBkw'
product_v2:
  - id: d830747e-f8f3-4fce-8eff-d53b333b1639
feature_v2:
  - id: a080bb92-ba2a-4e53-ba60-f5184d1a9e9a
source-git-commit: f7262325ce9f76ff99dbcb6e3e0519d2f2aaeaa5
workflow-type: tm+mt
source-wordcount: 31
ht-degree: 3%

---


# 概要

テスト -

>[!VIDEO](https://video.tv.adobe.com/v/3502250/?captions=jpn&quality=12&learn=on){transcript=true}
